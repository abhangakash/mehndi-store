...
export default function LoginPage() {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    if (error) toast.error(error.message)
    else { toast.success('Welcome!'); router.push('/') }
    setLoading(false)
  }
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12" style={{ backgroundColor: 'var(--brand-surface)' }}>
      <div className="w-full max-w-md">
        ...
        <div className="card p-6">
            ...
            <form onSubmit={handleEmailLogin} className="flex flex-col gap-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="Your password" style={{ paddingLeft: '2.5rem', paddingRight: '2.5rem' }} />
            ...
            <form onSubmit={otpSent ? handleVerifyOtp : handleSendOtp} className="flex flex-col gap-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="9876543210" maxLength={10} style={{ paddingLeft: '4rem' }} disabled={otpSent} />
                </div>
              </div>
              {otpSent && (
                <div>
          ...
          <Link href="/track-order" className="flex items-center justify-center gap-2 w-full py-2.5 rounded-lg text-sm font-medium border hover:bg-gray-50"
          ...
          <p className="text-center text-sm mt-4" style={{ color: 'var(--brand-muted)' }}>
            ...
            <Link href="/signup" className="font-medium" style={{ color: 'var(--brand-green)' }}>Sign up free</Link>
          </p>
        </div>
      </div>
    </div>
