...
import { User, Package, LogOut, Edit2, Save, X, MapPin, Phone, Mail, Clock, Truck, CheckCircle, XCircle } from 'lucide-react'
    ...
    <div className="min-h-screen py-10 px-4" style={{ backgroundColor: 'var(--brand-surface)' }}>
      <div className="max-w-3xl mx-auto">
        <div className="card p-6 mb-5 flex items-center gap-4">
          <div className="w-16 h-16 rounded-full flex items-center justify-center text-white text-xl font-semibold flex-shrink-0" style={{ backgroundColor: 'var(--brand-green)' }}>{initials}</div>
          <div className="flex-1 min-w-0">
            <h1 className="text-lg font-semibold truncate" style={{ color: 'var(--brand-text)' }}>{profile?.full_name || 'My Account'}</h1>
            <p className="text-sm truncate" style={{ color: 'var(--brand-muted)' }}>{user.email || user.phone}</p>
          </div>
          <button onClick={handleSignOut} className="flex items-center gap-1.5 text-sm px-3 py-1.5 rounded-lg border hover:bg-red-50 text-red-500" style={{ borderColor: '#fca5a5' }}>
            <LogOut size={14} /> Sign out
          </button>
        </div>

        <div className="flex gap-2 mb-5">
          {[{ key: 'profile', label: 'Profile', icon: <User size={15} /> }, { key: 'orders', label: 'My Orders', icon: <Package size={15} /> }].map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-all"
              style={{ backgroundColor: tab === t.key ? 'var(--brand-green)' : 'white', color: tab === t.key ? 'white' : 'var(--brand-muted)', border: `1px solid ${tab === t.key ? 'var(--brand-green)' : 'var(--brand-border)'}` }}>
              {t.icon} {t.label}
            </button>
          ))}
        </div>
        ...
        {tab === 'profile' && (
          <div className="card p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              {!editing
                ? <button onClick={() => setEditing(true)} className="flex items-center gap-1.5 text-sm px-3 py-1.5 rounded-lg" style={{ color: 'var(--brand-green)', border: '1px solid var(--brand-green)' }}><Edit2 size={13} /> Edit</button>
                : <div className="flex gap-2">
                    <button onClick={() => setEditing(false)} className="flex items-center gap-1 text-sm px-3 py-1.5 rounded-lg border" style={{ borderColor: 'var(--brand-border)', color: 'var(--brand-muted)' }}><X size={13} /> Cancel</button>
                    <button onClick={handleSaveProfile} disabled={saving} className="flex items-center gap-1 text-sm px-3 py-1.5 rounded-lg text-white" style={{ backgroundColor: 'var(--brand-green)' }}><Save size={13} /> {saving ? 'Saving...' : 'Save'}</button>
                  </div>
              }
            </div>
            <div className="flex flex-col gap-4">
              <div>
                ...
                {editing ? <input value={form.full_name} onChange={e => setForm(f => ({ ...f, full_name: e.target.value }))} placeholder="Your name" />
              ...
              <div>
                ...
                <p className="text-sm py-2.5 px-4 rounded-lg" style={{ backgroundColor: 'var(--brand-surface)', color: 'var(--brand-muted)' }}>{user.email || '—'}</p>
              </div>
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: 'var(--brand-text)' }}>Phone</label>
                {editing ? <input value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} placeholder="9876543210" />
        ...
        {tab === 'orders' && (
          <div>
            {ordersLoading ? (
              <div className="card p-10 text-center"><div className="w-8 h-8 border-2 border-t-transparent rounded-full animate-spin mx-auto" style={{ borderColor: 'var(--brand-green)' }} /></div>
            ) : orders.length === 0 ? (
                  ...
                  return (
                    <div key={order.id} className="card overflow-hidden">
                      <div className="p-4 border-b flex items-start justify-between gap-3" style={{ borderColor: 'var(--brand-border)' }}>
                        ...
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium" style={{ backgroundColor: config.bg, color: config.color }}>
                      ...
                      <div className="p-4 flex items-start gap-2">
                        ...
                        <p className="text-xs" style={{ color: 'var(--brand-muted)' }}>{order.address}, {order.city} — {order.pincode}</p>
                      </div>
                    </div>
                  )
                })}
