    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: 'var(--brand-surface)' }}>
        <div className="text-center">
          ...
          <p className="text-sm" style={{ color: 'var(--brand-muted)' }}>Loading...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="card p-6 mb-5 flex items-center gap-4">
          ...
          <button onClick={handleSignOut}
            className="flex items-center gap-1.5 text-sm px-3 py-1.5 rounded-lg border transition-colors hover:bg-red-50 text-red-500"
              ...
              {!editing ? (
                ...
                <div className="flex gap-2">
                  <button onClick={() => setEditing(false)}
                    className="flex items-center gap-1 text-sm px-3 py-1.5 rounded-lg border transition-colors hover:bg-gray-50"
                    style={{ borderColor: 'var(--brand-border)', color: 'var(--brand-muted)' }}>
                    <X size={13} /> Cancel
                  </button>
                  <button onClick={handleSaveProfile} disabled={saving}
                    className="flex items-center gap-1 text-sm px-3 py-1.5 rounded-lg text-white transition-colors"
                    style={{ backgroundColor: 'var(--brand-green)' }}>
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                {editing ? (
                  ...
                  <p className="text-sm py-2.5 px-4 rounded-lg" style={{ backgroundColor: 'var(--brand-surface)', color: 'var(--brand-text)' }}>
                    {profile?.full_name || '—'}
                  </p>
                )}
              </div>
            ...
            {ordersLoading ? (
              <div className="card p-10 text-center">
                <div className="w-8 h-8 border-2 border-t-transparent rounded-full animate-spin mx-auto mb-2"
                  style={{ borderColor: 'var(--brand-green)' }} />
                <p className="text-sm" style={{ color: 'var(--brand-muted)' }}>Loading orders...</p>
              </div>
            ) : orders.length === 0 ? (
              <div className="card p-10 text-center">
                ...
                <p className="text-sm mb-4" style={{ color: 'var(--brand-muted)' }}>Your order history will appear here</p>
