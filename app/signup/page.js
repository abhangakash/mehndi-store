...
export default function SignupPage() {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    if (error) {
      ...
      toast.success('Welcome to Shrilekha!')
      router.push('/')
    }
    setLoading(false)
  }
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="tel" value={phone} onChange={e => setPhone(e.target.value)}
                      placeholder="9876543210" maxLength={10} style={{ paddingLeft: '4rem' }} />
                  </div>
                </div>
              )}
              {otpSent && (
                <div>
                  ...
                  <p className="text-xs mt-1.5" style={{ color: 'var(--brand-muted)' }}>
                    ...
                    <button type="button" onClick={() => setOtpSent(false)}
                      className="underline" style={{ color: 'var(--brand-green)' }}>Change</button>
                  </p>
                </div>
              )}
              <button type="submit" disabled={loading} className="btn-primary w-full justify-center mt-1">
