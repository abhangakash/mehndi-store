      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <div className="inline-block mt-3 px-4 py-1.5 rounded-full text-sm font-medium"
            style={{ backgroundColor: 'var(--brand-surface)', color: 'var(--brand-muted)', border: '1px solid var(--brand-border)' }}>
        ...
        <div className="card mb-4 overflow-hidden">
          <div className="px-5 py-4 border-b" style={{ borderColor: 'var(--brand-border)', backgroundColor: 'var(--brand-surface)' }}>
            <h2 className="font-semibold text-sm flex items-center gap-2" style={{ color: 'var(--brand-text)' }}>
          ...
          <div className="p-5">
            ...
            <div className="mt-3 pt-3 border-t flex flex-col gap-1.5" style={{ borderColor: 'var(--brand-border)' }}>
              <div className="flex justify-between text-sm">
                ...
                <span style={{ color: order.shipping_amount == 0 ? '#15803d' : 'var(--brand-text)' }}>
                  {order.shipping_amount == 0 ? 'FREE' : `₹${order.shipping_amount}`}
                </span>
              </div>
              <div className="flex justify-between font-semibold">
                <span style={{ color: 'var(--brand-text)' }}>Total</span>
                <span style={{ color: 'var(--brand-brown)' }}>₹{Number(order.total_amount).toFixed(0)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="card p-5 mb-4">
          <h2 className="font-semibold text-sm mb-3 flex items-center gap-2" style={{ color: 'var(--brand-text)' }}>
          ...
          <div className="flex items-center gap-1.5 mt-2 text-sm" style={{ color: 'var(--brand-muted)' }}>
        ...
        <div className="flex flex-col gap-3">
          <a href={`https://wa.me/919623740541?text=${waMsg}`} target="_blank" rel="noreferrer"
            className="flex items-center justify-center gap-2 w-full py-3 rounded-lg text-sm font-medium border hover:bg-green-50 transition-colors"
            style={{ borderColor: '#86efac', color: '#15803d' }}>
            <Phone size={15} /> Share order on WhatsApp
          </a>
          <Link href="/track-order" className="btn-secondary w-full justify-center text-sm">
            Track My Order <ArrowRight size={15} />
          </Link>
          <Link href="/products" className="btn-primary w-full justify-center text-sm">
